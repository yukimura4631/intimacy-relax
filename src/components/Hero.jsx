  ...
  return (
    <section className="pt-20 relative h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="relative z-10 text-center px-4 max-w-2xl">
        <h2 className="text-6xl md:text-7xl font-light text-purple-900 mb-4 tracking-wider">
          INTIMACY
        </h2>
        <p className="text-xl md:text-2xl text-purple-700 mb-6 font-light">
          寄り添いの癒し
        </p>
        <p className="text-gray-700 mb-8 leading-relaxed">
          疲れた時に気兼ねなく足を運べる空間。<br />
          あなたの心と身体に寄り添う、完全個室のリラクゼーション。
        </p>
        <button
          onClick={() => setBookingModal(true)}
          className="px-8 py-3 bg-purple-700 text-white rounded-full hover:bg-purple-800 transition-all shadow-lg hover:shadow-xl text-sm tracking-wide"
        >
          予約する
        </button>
      </div>
        ...
        {heroImages.map((_, index) => (
          <button
            ...
            className={`h-2 rounded-full transition-all ${
              index === currentSlide ? 'w-8 bg-purple-700' : 'w-2 bg-purple-300'
            }`}
          />
        ))}
      </div>
